...
int EndCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    // Recreate the signature of the session id and compare with the signature contained in the token
    char signatureCheck[SIGNATURE_STRLEN + 1];
    signData((const unsigned char *)signKey, signKeyLen, (const unsigned char *)sessionId, SESSION_ID_STRLEN, signatureCheck);

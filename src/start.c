...
int StartCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    // Open a ticket and call me paranoid. You better like me being paranoid.
    while (1)
    {
    ...
    signData((const unsigned char *)signKey, signKeyLen, (const unsigned char *)sessionId, SESSION_ID_STRLEN, signature);
    ...
    RedisModule_ReplyWithString(ctx, RedisModule_CreateStringPrintf(ctx, "%s.%s.%s", TOKEN_VERSION, sessionId, signature));
